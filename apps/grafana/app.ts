...
class Grafana extends Chart {
  constructor(scope: Construct, id: string) {
    ...
    new Helm(this, "grafana", {
      ...
      values: {
        ...
        securityContext: {
          // avoid a recursive chown of the volume on every restart
        ...
        initChownData: {
          ...
          // CephFS honors fsGroup, so this legacy chown-on-every-start init
          // container (for CSI drivers that don't) is unneeded and would
          // otherwise still force a recursive chown despite
          // securityContext.fsGroupChangePolicy above
          enabled: false,
        },
      },
    });
  ...
  private loadDashboardsFromDir(rootDir: string, dir: string, ns: string) {
    ...
    for (const entry of entries) {
      ...
      // gcx wraps dashboards in apiVersion/kind/spec — extract the inner spec
